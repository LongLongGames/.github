...
- 同一 `mp_account_id` 仅允许一个有效登录会话（与渠道无关：official / guest / Steam 等均适用）。
- 新设备（或同账号重新）`/auth/login` 成功时，服务端作废该账号下其它 `device_id` 的 refresh_token。
- 旧设备持有的 access/refresh 均失效：业务请求 401；refresh 失败后客户端 Logout → Login。
- 客户端不实现「多端检测」；仅遵循：401 → 尝试 refresh 一次 → 失败再登出。
